...
export function CommentSection() {
  ...
  const addComment = (parentId: number | null, content: string, userId: number) => {
  ...
  return (
    <div className="max-w-2xl mx-auto mt-8">
      ...
      <div className="mb-4">
        <Select onValueChange={(value) => setCurrentUser(users.find(u => u.id === parseInt(value)) || users[0])}>
      ...
      {comments.map((comment) => (
        <Comment key={comment.id} {...comment} onReply={addComment} currentUser={currentUser} />
      ))}
      <div className="mt-4">
        <Textarea
          value={newComment}
